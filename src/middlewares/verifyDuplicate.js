...
export const checkDuplicateEstadoKaizen = async (req, res, next) => {
    ...
    if (query) return res.status(201).json({ message: `El Estado ${name} ya existe` });

    next();
};

export const checkDuplicateEstadoTDPKaizen = async (req, res, next) => {
    const { name } = req.body;
    const query = await EstadoTDPKaizen.findOne({ name });
