...
const estadoController = {
    getall: async (req, res) => {
        try {
            const query = await EstadoTDPKaizen.find().sort({ valor: 1 });
    ...
    getActive: async (req, res) => {
        try {
            const query = await EstadoTDPKaizen.find({ estado: true }).sort({ valor: 1 });
    ...
    getaOneById: async (req, res) => {
        ...
        try {
            const query = await EstadoTDPKaizen.findById(itemId);
    ...
    createOne: async (req, res) => {
        ...
        try {
            const newObj = new EstadoTDPKaizen({
                name,
                valor,
                estado,
            });
    ...
    updateOneById: async (req, res) => {
        ...
        try {
            const query = await EstadoTDPKaizen.findByIdAndUpdate(itemId, { name, valor, estado });
    ...
    deleteOneById: async (req, res) => {
        ...
        try {
            const query = await EstadoTDPKaizen.findByIdAndDelete(itemId);
