...
itemController.getAll = async(req, res) => {
    try {
        const query = await UbicacionMejoraKaizen.find().sort({ name: 1 });
        if (query.length > 0) {
...
itemController.getOneById = async(req, res) => {
    ...
    try {
        const query = await UbicacionMejoraKaizen.findById(itemId);
        if (query) {
            res.json({one: query});
        } else {
...
itemController.getAllActivos = async(req, res) => {
    try {
        const query = await UbicacionMejoraKaizen.find({ estado: true }).sort({ name: 1 });
        if (query.length > 0) {
            res.json({total: query.length, all: query});
        } else {
...
itemController.createOne = async(req, res) => {
    ...
    try {
        const obj = new UbicacionMejoraKaizen({ cod_interno,name, description, estado });
...
itemController.updateOneById = async(req, res) => {
    ...
    const { itemId } = req.params;
    try {
        const query = await UbicacionMejoraKaizen.findByIdAndUpdate(itemId, { name, description, estado });
...
itemController.deleteOneById = async(req, res) => {
    const { itemId } = req.params;
    try {
        const query = await UbicacionMejoraKaizen.findByIdAndDelete(itemId);
        if (query) {
