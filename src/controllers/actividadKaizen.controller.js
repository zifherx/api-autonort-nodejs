...
const activityController = {
    ...
    updateOneById: async (req, res) => {
        ...
        try {
            ...
            if (estadoTDP_E == null || estadoTDP_E == undefined) {
                estadoTDPNull = null;
            } else {
                const estadoTDPFound = await EstadoTDPKaizen.findOne({ name: estadoTDP_E });
                if (!estadoTDPFound) return res.status(404).json({ message: `Estado ${estadoTDP_E} no encontrado` });
                estadoTDPNull = estadoTDPFound._id;
            }
    ...
    sendActivityToEvaluate: async (req, res) => {
