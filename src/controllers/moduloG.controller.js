...
moduloController.getAll = async (req, res) => {
    try {
        const query = await ModuloG.find().sort({ name: 1 }).populate({
            path: "menuG",
            select: "name icon",
        });
...
moduloController.getOneById = async (req, res) => {
    ...
    try {
        const query = await ModuloG.findById(moduloId).populate({
            path: "menuG",
            select: "name icon",
        });
...
moduloController.getAllByActivos = async (req, res) => {
    try {
        const query = await ModuloG.find({ estado: true }).sort({ name: 1 }).populate({
            path: "menuG",
            select: "name icon",
        });
